...
const ModalUpdateCategory = ({ visible, onCancel, ...rest }) => {
  const { Item } = Form;
  return (
    <Modal
      ...
      onCancel={onCancel}
      width={450}
      className="default-modal category-modal"
    >
      <Form>
        <div>
          <Row gutter={[8,8]}>
            <Col span={6}>
              <span style={{position:"relative",top:"0.25rem"}}>Tên nhóm</span>
            </Col>
            <Col span={18}>
              <Item
                name="name"
                rules={[{ required: true, message: "Bạn chưa nhập tên nhóm" }]}
              >
                <InputString />
              </Item>
            </Col>
